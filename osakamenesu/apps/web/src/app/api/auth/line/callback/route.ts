 ...
 *
 * LINEからのOAuthコールバックを処理し、バックエンドAPIにcode/stateを転送。
...
async function getHandler(request: NextRequest) {
  ...
  if (error) {
    ...
    const errorUrl = new URL('/therapist?error=line_auth_failed', request.url)
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.error('Backend LINE callback failed:', response.status, errorData)
