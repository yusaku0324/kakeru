...
import { normalizeId, isClientMockMode, FAVORITES_MOCK_COOKIE_NAME } from '../lib/utils'
...
export function TherapistFavoritesProvider({ children }: { children: React.ReactNode }) {
  const initialFavorites = useMemo(() => {
    ...
    const rawCookie = document.cookie
      ?.split(';')
      .map((part) => part.trim())
      .find((item) => item.startsWith(`${FAVORITES_MOCK_COOKIE_NAME}=`))
    if (!rawCookie) {
      return new Map<string, TherapistFavoriteRecord>()
    }
    const payload = rawCookie.slice(FAVORITES_MOCK_COOKIE_NAME.length + 1)
    try {
  ...
  useEffect(() => {
    ...
    if (res.status === 401) {
      ...
      return
        ...
        const data = (await res.json()) as TherapistFavoriteApiResponse[]
  ...
  const toggleFavorite = useCallback(
    async ({ therapistId, shopId }: ToggleFavoritePayload) => {
