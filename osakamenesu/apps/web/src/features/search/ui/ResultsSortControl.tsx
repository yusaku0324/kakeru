...
export function ResultsSortControl({ options, currentSort = 'recommended', hashTarget = 'search-results' }: ResultsSortControlProps) {
  const router = useRouter()
  const pathname = usePathname() || '/search'
  const searchParams = useSearchParams()

  const normalizedParams = useMemo(() => new URLSearchParams(searchParams?.toString() ?? ''), [searchParams])
  ...
  return (
    <label className="flex items-center gap-2 text-xs font-semibold text-neutral-text">
      並び替え
      <select className={clsx(SELECT_CLASS)} value={currentSort} onChange={(event) => handleChange(event.target.value)}>
        {options.map((option) => (
