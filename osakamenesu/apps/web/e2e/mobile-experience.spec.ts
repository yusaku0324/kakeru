 ...
 *
 * These tests run on mobile-chrome and mobile-safari projects defined in playwright.config.ts
 */

// Skip tests on desktop browsers
...
test.describe('Mobile Experience', () => {
  ...
  test('PWA installation prompt', async ({ page }, testInfo) => {
    await page.goto('/')
    ...
    // Check iOS specific meta tags for Safari projects
    const projectName = testInfo.project.name
  ...
  test('mobile-specific UI elements', async ({ page }, testInfo) => {
    await page.goto('/')
    ...
    // Check for bottom navigation on phones (not iPad-like projects)
    const projectName = testInfo.project.name
