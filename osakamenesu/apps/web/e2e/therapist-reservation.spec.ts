...
test('therapist card navigates to staff page and reservation can be sent', async ({
  page,
  baseURL,
}) => {
  ...
  const noteField = page
    .getByLabel('ご要望・指名など 任意', { exact: false })
    .or(page.getByPlaceholder('指名やオプションの希望などがあればご記入ください'))
  await noteField.fill('Playwright からのリクエストです')

  await page.getByRole('button', { name: '予約リクエストを送信' }).click()

  await expect(
    page.getByText('送信が完了しました。店舗からの折り返しをお待ちください。'),
  ).toBeVisible()
})
