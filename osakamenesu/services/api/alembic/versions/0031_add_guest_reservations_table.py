...
Revision ID: 0031_add_guest_reservations
...
revision = '0031_add_guest_reservations'
...
def upgrade():
    ...
    op.create_table(
        ...
        sa.Column('therapist_id', postgresql.UUID(as_uuid=True), sa.ForeignKey('therapists.id', ondelete='SET NULL'), nullable=True),
        ...
        sa.UniqueConstraint('therapist_id', 'start_at', 'end_at', name='uq_guest_reservations_therapist_slot'),
    )

    op.create_index('ix_guest_reservations_shop_id', 'guest_reservations', ['shop_id'])
    op.create_index('ix_guest_reservations_therapist_id', 'guest_reservations', ['therapist_id'])
    op.create_index('ix_guest_reservations_start_at', 'guest_reservations', ['start_at'])
    ...
    op.create_index('ix_guest_reservations_base_staff_id', 'guest_reservations', ['base_staff_id'])


def downgrade():
    op.drop_index('ix_guest_reservations_base_staff_id', table_name='guest_reservations')
    ...
    guest_status.drop(op.get_bind(), checkfirst=True)
