...
def _to_doc(
    p: models.Profile, today: bool = False, tag_score: float = 0.0, ctr7d: float = 0.0
) -> dict:
...
async def create_profile(
    ...
    slug = await _ensure_unique_slug(
        db, payload.slug or payload.name, fallback_seed=uuid.uuid4()
    )
    p = models.Profile(
        slug=slug,
        name=payload.name,
...
async def get_profile_detail(profile_id: str, db: AsyncSession = Depends(get_session)):
    try:
        detail = await profile_service.get_profile_detail(
            db=db,
